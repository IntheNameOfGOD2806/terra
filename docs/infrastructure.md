# K8s Cluster on AWS — Infrastructure Documentation

> **Project**: Self-managed Kubernetes cluster on AWS  
> **Region**: `ap-southeast-1` (Singapore)  
> **IaC Tool**: Terraform (AWS Provider `v5.21.0`)  
...
9. [Provisioning Pipeline](#9-provisioning-pipeline)
10. [Terraform File Map](#10-terraform-file-map)
...
- **Private subnet** instances have **no public IPs**. They are accessed via:
  ...
  - **Internet egress**: Via a NAT Gateway (pre-existing, outside Terraform scope).

---

## 3. Compute Resources

All EC2 instances use **Ubuntu AMI** (`ami-00d8fc944fb171e29`) and the `k8s` SSH key pair.
...
| Attribute           | Value                                          |
|---------------------|-------------------------------------------------|
| **Resource**        | `aws_instance.k8s_worker` (count = 3)          |
| **Instance Type**   | `t2.medium`                                     |
| **Root Volume**     | 20 GB `gp3`                                    |
| **Subnet**          | `private-subnet-1(real)` (**no public IP**)      |
...
**Purpose**: Run application workloads. Each worker joins the cluster using a `join-command.sh` generated by the master node. Workers expose services via **NodePort `:32222`** (mapped to the Ingress Controller).

### 3.5 NFS Servers (×2)

| Attribute           | Value                                          |
|---------------------|-------------------------------------------------|
| **Resource**        | `aws_instance.k8s_nfs` (count = 2)             |
| **Instance Type**   | `t2.medium`                                     |
| **Root Volume**     | 30 GB `gp3`                                    |
| **Subnet**          | `private-subnet-1(real)` (**no public IP**)      |
| **Security Group**  | `k8s_nfs`                                       |
| **SSH Access**      | Via Master 1 as proxy (Ansible `ProxyCommand`)  |

**Purpose**: Provide **NFS-based persistent storage** (`PersistentVolume` / `PersistentVolumeClaim`) for the Kubernetes cluster. Ansible installs NFS packages via the master node's SSH tunnel.
...
| Attribute           | Value                              |
|---------------------|------------------------------------|
| **Resource**        | `aws_lb_target_group.k8s_tg_lb`   |
| **Name**            | `k8s-tg-lb`                       |
...
| Service            | Namespace     | Deployment Method    | Description                             |
|--------------------|---------------|----------------------|-----------------------------------------|
| **Rancher**        | —             | Docker container     | Kubernetes management UI                |
| **ArgoCD**         | `argocd`      | `kubectl apply`      | GitOps continuous delivery              |
| **Prometheus**     | `monitoring`  | Helm chart           | Metrics collection                      |
| **Grafana**        | `monitoring`  | Helm chart           | Metrics visualization / dashboards      |
| **Elasticsearch**  | `logging`     | Helm chart (v7.17)   | Log aggregation and search              |
| **FileBrowser**    | —             | Ansible              | Web-based file manager on Nginx LB      |
| **EBS CSI Driver** | `kube-system` | Helm chart           | Dynamic EBS volume provisioner          |
...
Nginx LB (Bastion) — Public IP
  ...
  └──▶ NFS × 2   (private IP via ProxyCommand through Master 1)
```
...
The infrastructure uses a **two-phase provisioning** approach:

### Phase 1: Terraform (`terraform apply`)

Terraform creates resources in the following dependency order:

```
1. Key Pair (k8s)
2. IAM Role + Policy + Instance Profile
3. Security Groups (all 4)
4. Nginx LB instance       ← provisioned with Docker, Nginx, FileBrowser
5. Master 1 instance        ← depends on Nginx LB
   └─ kubeadm init, Rancher, ArgoCD, Helm, EBS CSI, monitoring stack
6. Master 2 instance        ← depends on Master 1
   └─ joins as HA control plane via bastion
7. Worker instances (×3)    ← depends on Master 1
   └─ joins cluster via bastion
8. NFS instances (×2)       ← depends on Master 1
   └─ NFS packages via Ansible through proxy
9. ALB + Target Group + Listener
   └─ attaches workers to target group on port 32222
```
...
|-----------------------------------|--------------------|-------------------------------------|
| `installDocker.yaml`             | Nginx LB, Masters  | Install Docker CE                   |
| `installNginx.yaml`             | Nginx LB           | Install and configure Nginx         |
| `installFileBrowser.yaml`       | Nginx LB           | Install FileBrowser web UI          |
| `installRancher.yaml`           | Master 1           | Deploy Rancher container            |
| `installHelm.yaml`              | Master 1           | Install Helm CLI                    |
| `installNFS.yaml`               | NFS Servers         | Install NFS kernel server           |
| `getJoinCommandk8s.yaml`        | Master 1           | Extract kubeadm join command        |
| `fetchKubeConfigfromMaster.yaml` | Master 1           | Copy kubeconfig to local machine    |
| `installAWScli.yaml`            | Various            | Install AWS CLI                     |
| `installCockPit.yaml`           | Various            | Install Cockpit web console         |
| `tailScale.yaml`                | Various            | Configure Tailscale VPN             |

### Bootstrap Scripts

| Script              | Target      | Description                                    |
|---------------------|-------------|------------------------------------------------|
| `master.sh`         | Master 1    | Full K8s master bootstrap (kubeadm init, CNI)  |
| `master-2.sh`       | Master 2    | Prepare node for HA control-plane join          |
| `worker.sh`         | Workers     | Prepare node for worker join                    |
| `join-command.sh`    | Workers     | Generated kubeadm worker join command           |
| `join-command-master.sh` | Master 2 | Generated kubeadm control-plane join command  |

---

## 10. Terraform File Map

| File                   | Purpose                                              |
|------------------------|------------------------------------------------------|
| `provider.tf`          | AWS provider configuration (`ap-southeast-1`)        |
| `var.tf`               | Variables: AMIs, instance types, counts, region       |
| `data.tf`              | Data sources: existing VPC, subnets, VPN SG           |
| `keypair.tf`           | SSH key pair resource                                 |
| `iam.tf`               | IAM Role, Policy attachment, Instance Profile         |
| `security_groups.tf`   | All 4 security groups + ingress/egress rules          |
| `main.tf`              | EC2 instances, ALB, Target Group, Listener, provisioners |
